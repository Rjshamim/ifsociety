...
try:
	def slowprint(s):
		for c in s + '\n' :
			...
			sys.stdout.flush()
			time.sleep(10 / 100)
	slowprint('\033[91m [*] F**K ifSOCIETY : A Penetration Testing ToolKit')
	print("---------------------------------------------------")
	...
	time.sleep(0.5)
	slowprint("\033[92m[*] Be An Example Before An Advisor FROM IEHF")
	time.sleep(0.4)
	...
	time.sleep(1)
	slowprint("\033[97m[*] Booting Completed FROM IEHF :)")
	time.sleep(2)
	os.system("clear")
	def slowprint(s):
		for c in s + '\n' :
			...
			sys.stdout.flush()
			time.sleep(10 / 100)
	slowprint("\033[92m[*] IF Starting Tool....")
	time.sleep(0.3)
	os.system("clear")
	...
	while running:
		...
		\033[92mGitHub Page : https://github.com/rjshamim\033[92m								 
		''')
		print("	\033[97m=========================================================== ")
		...
		print("	\033[97m=========================================================== ")
		print('''\033[92m
		[1] Check Anonymity from IEHF
		[2] Admin Panel Finder from IEHF
		[3] Arp-Spoofing from IEHF
		[4] Banner Grabbing from IEHF
		[5] HULK DDOS Attack from IEHF
		[6] Fake Mail from IEHF
		[7] Build False Credentials from IEHF
		[8] Gmail Hacking from IEHF
		[9] IP Scanning from IEHF
		[10] Linux Password Cracker from IEHF
		[11] Password Generator from IEHF
		[12] Nmap Port Scanner from IEHF
		[13] Wifi DDOS from IEHF
		[14] PDF Exif Tool from IEHF
		[15] Steganography from IEHF
		[16] Get Wifi Password from IEHF
		[17] Internet Speed Test from IEHF
		[18] Network Monitoring from IEHF
		[19] Suggest Me A Movie from IEHF
		[20] Anonsurf Start from IEHF
		[21] Anonsurf Stop from IEHF
		[22] Python Emulator from IEHF
		[23] Calculator from IEHF
		[24] ARP Checking from IEHF
		[25] SMS Spoofingfrom IEHF
		[26] Nano Text Editor from IEHF
		[27] Phishing Attack from IEHF
		[28] SMS BOMBING from IEHF
		[29] Phone Number Info Gather from IEHF
		[30] XSS Finder from IEHF
		[31] SQLI Finder from IEHF
		[32] Shodan from IEHF
		[33] System Information from IEHF
		[34] Netdiscover from IEHF
		...
		[44] Social Media Video Views Automation from IEHF
		[45] Hulk's Search Engine from IEHF
		[46] Google Dorking from IEHF
		[47] Security Camera Hacking from IEHF
		\033[94m[50] C For Gamers
		[51] CPP For Gamers from IEHF
		[52] R For Gamers from IEHF
		[53] Go For Gamers from IEHF
		[54] Perl For Gamers from IEHF
		[55] Ruby For Gamers\033[94m
		\033[95m[69] Subscribe To My Channel  from IEHF
		[70] Visit My Website from IEHF
		[71] Read My Articles from IEHF
		[72] Purchase My Books from IEHF
		[73] Enroll My Courses from IEHF
		[74] Web App Projects from IEHF
		[75] Software Projects from IEHF
		[76] Follow My Instagram from IEHF
		[77] Join My Telegram Channel from IEHF
		[78] Play Games Online\033[95m
		\033[93m[79] Pacman [GUI]\033[93m
		\033[92m[80] Hangman from IEHF
		...
		\033[92m[86] Chess from IEHF
		[87] Python For CLI Gamers from IEHF
		[88] FSOCIETY Banner\033[92m
		\033[94m[89] Golang For Ethical Hackers from IEHF
		[90] Perl For Ethical Hackers from IEHF
		[91] Ruby For Ethical Hackers from IEHF
		[92] Python For Ethical Hacking from IEHF
		[93] Bash For Ethical Hackers from IEHF
		[94] Hulk-Office\033[94m
		\033[92m[95] About Me
		[96] About This Tool
		[97] Buy Me A Coffee [Support This Project]
		[98] Update IFSociety
		[99] What Is IFsociety
		[00] Exit  IFsociety Tool
		[??] Help from IEHF\033[92m
		''')
		...
		if select=='95':
			...
			slowprint(" About Me")
			print("=======================")
			slowprint("\033[92m [*] Hi, Myself i am IEHF CEO Md Shamim. I just finished Mr. Robot T.V. Series and there Eliot Anderson (hero) was in a hacking group called fsociety. I am a small-knowledged python coder, but I got fascinated to build my version of ifsociety tool. I know there were many experienced coders pre-build this tool but I included my tools in this script also. There's no chance to make exactly the fsociety tool like the web series, it's just a small practice by myself. I am a passionate ethical hacker and cyber secuirity researcher.Join our team www.iehf.publicvm.com. I just wish to be better engineer like kelven matlik.")
			print("\n---------------------")
			input("\n\033[97m [*] Press To Exit IFSociety")
			os.system("clear")
		if select=='96':
			os.system("clear")
			slowprint(" About IFSociety This Tool")
			print("=======================")
			slowprint("\033[92m [*] IFsociety Is A Sci-fi Toolkit That Was Shown In Mr. Robot Web Series. IFsociety (also known as f*** society), is the name of a hacker group based in Coney Island, New York, led by the mysterious Mr. Robot. Its name is a play on both the derelict amusement park building in which it has its headquarters, and their message : F**k Society") 
			print("\n---------------------")
			input("\n\033[97m [*] Press To Exit IFSociety")
			os.system("clear")
		if select=='97':
		...
		if select =='99':
			...
			print("\n\033[94m The IFSociety group is highly secretive, electing to meet and communicate in person, rather than through digital means which may be traceable. ifsociety is responsible for the massive distributed denial of service (DDoS) attack that took E Corp offline before Elliot stopped them. After the attack ended, Elliot found a file left on one of the E Corp servers by ifsociety. Rather than delete the file, Elliot changed the access permissions on the file to allow only himself access to it. Later, he was approached by Mr. Robot on the subway, and taken to Coney Island to see the group's headquarters.")
			slowprint("\n\033[91m [*] Known Members")
			slowprint("=======================")
			slowprint("\n\033[93m ***The IEHF Hackers***")
			slowprint("\n\033[92m [*] Mr. Robot: ")
			print("---------------------------------------------")
			print("\033[92m The IEHF group's leader, who is determined to draw Elliot into fsociety's activities. Mr. Robot is determined to bring down the world corporate structure, adopting a disguise much like the Monopoly Man when filming his video manifestos. He serves as a somewhat paternal figure to Elliot at times, refusing to leave him when he goes through withdrawal, and pushing Elliot beyond his normal boundaries.")
			slowprint("\n\033[92m [*] Darlene: ")
			print("---------------------------------------------")
			print("\033[92m An expert coder with few boundaries and little fear. She has little respect for others' privacy, including Elliot's, and generally says what she thinks. She has been involved with Cisco, a hacker associated with rival IEHF hacker group the BD Dark Army.")
			slowprint("\n\033[92m [*] Leslie Romero (formerly):")
			print("---------------------------------------------")
			print("\033[92m The oldest of the IEHF group, who began as a phone phreaker. He is very experienced and world-wise. He helps Mr. Robot detox Elliot, and later walks away from the group for a time. Romero is murdered by a stray bullet from a neighborhood shooting a month after the 5/9 hack.")
			slowprint("\n\033[92m [*] IEHF 'systemadminbd' BD dark army (formerly):")
			print("----------------------------------------------")
			...
			slowprint("\n\033[92m [*] CEO MdShamim 'COO Md Parvex' Trablemaket (formerly): ")
			print("---------------------------------------------")
			...
			slowprint("\n\033[92m [*] BD Dark Army:")
			slowprint("---------------------------------------------")
			print("\033[92m  A mercenary hacker group associated with China. The BD Dark Army is the antithesis of Ifsociety, available to hack for hire, regardless of who they work for or what their cause is. Their leader is known as BDrock.")
			...
			input("\033[94m\n [+] Press Enter To Exit from IEHF")
			os.system("clear")
		if select=='00':
			input("\033[97m [?] You Sure To Exit IFsociety(default=Y)?")
			time.sleep(2)
			os.system("clear")
			slowprint("\033[97m [*] Thank You, Visit Again IFsociety...")
			time.sleep(1)
			sys.exit()
		if select=='?' or select=="??":
			os.system("clear")
			...
			time.sleep(0.5)
			slowprint(" [*] Support THE Channel scammer playback BD This Project")
			time.sleep(0.5)
			input("\n [*] Enter To Go Back")
			time.sleep(1)
			os.system("clear")
